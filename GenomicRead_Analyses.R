...
N_Genomic_Fig1 <- All_pipeSummary %>% 
...
P_Genomic_Fig1 <- All_pipeSummary %>% 
  ggplot(aes(x = Type, y = P_Genomic)) + 
  geom_boxplot(fill="grey", width = 0.6, outlier.size = 0.9, alpha = 0.2) + 
  geom_point(aes(fill = Type), shape = 21, alpha = 0.8, size = 1.7, position = position_jitter(0.2)) + 
  # geom_point(shape = 16, alpha = 0.8, size = 1.5, position = position_jitter(0.2)) + 
  labs(title = "PredictTB Run 1: P_Genomic for all sample types",
       subtitle = "Marmoset from ProbeTest3", 
       x = "Sample type", 
...
ggsave(P_Genomic_Fig1,
       file = paste0("P_Genomic_Fig1.pdf"),
       path = "Figures/GenomicRead_Analyses",
       width = 8, height = 5, units = "in")
...
TenReads_Fig1 <- All_pipeSummary %>% 
  ggplot(aes(x = Type, y = AtLeast.10.Reads)) + 
  geom_boxplot(fill="grey", width = 0.6, outlier.size = 0.9, alpha = 0.2) + 
  geom_point(aes(fill = Type), shape = 21, alpha = 0.8, size = 1.7, position = position_jitter(0.2)) + 
  # geom_point(shape = 16, alpha = 0.8, size = 1.5, position = position_jitter(0.2)) + 
  geom_hline(yintercept = 4499*0.8, linetype = "dashed", alpha = 0.5) + 
  annotate("text", x = 0.9, y = 4499*0.8, label = "80%", hjust = 1, vjust = -0.5, color = "black") + 
  geom_hline(yintercept = 4499*0.5, linetype = "dashed", alpha = 0.5) + 
  annotate("text", x = 0.9, y = 4499*0.5, label = "50%", hjust = 1, vjust = -0.5, color = "black") + 
  labs(title = "PredictTB Run 1: Genes with >= 10 reads aligning for all sample types",
       subtitle = "Marmoset from ProbeTest3", 
       x = "Sample type", 
...
ggsave(TenReads_Fig1,
       file = paste0("TenReads_Fig1.pdf"),
       path = "Figures/GenomicRead_Analyses",
       width = 8, height = 5, units = "in")

# What if I did this one but removed the non Rv genes?
# Would need to go in the TPM and manually count, put this to the side for now
